...
def load_user_prompts(prompts_file):
    """
    加载用户prompts
    ...
    with open(prompts_file, 'r', encoding='utf-8') as f:
        for line in f.readlines():
            ...
            prompts.append(user_data['prompt'])
    return prompts


def generate_user_profiles(prompts_file, system_prompt_file, output_file, 
    ...
    # 2. 加载用户prompts
    print(f"\n[2/4] 加载用户prompts: {prompts_file}")
    all_prompts = load_user_prompts(prompts_file)
    print(f"   总用户数: {len(all_prompts)}")
    ...
    if sample_size and sample_size < len(all_prompts):
        ...
        print(f"   处理范围: {start_index} - {end_index-1} (共 {len(selected_indices)} 个用户)")
    else:
        selected_indices = list(range(start_index, len(all_prompts)))
        selected_prompts = [all_prompts[i] for i in selected_indices]
        print(f"   处理范围: {start_index} - {len(all_prompts)-1} (共 {len(selected_indices)} 个用户)")
...
def test_single_user(prompts_file, system_prompt_file, user_index=None, model='qwen-turbo'):
    """
    测试单个用户的生成效果
    ...
    if user_index is None:
        user_index = np.random.choice(len(prompts))
    
    selected_prompt = prompts[user_index]
    
    # 显示信息
    print(Colors.GREEN + "=" * 60 + Colors.END)
    print(Colors.GREEN + f"测试用户画像生成 (用户索引: {user_index})" + Colors.END)
    print(Colors.GREEN + "=" * 60 + Colors.END)
...
if __name__ == '__main__':
    ...
    SYSTEM_PROMPT_FILE = f'generation/user/user_system_prompt_{DATASET}.txt'
    PROMPTS_FILE = f'dataset/{DATASET}/processed/user_llm_input.json'
    OUTPUT_FILE = f'dataset/{DATASET}/processed/user_profiles.json'
    ...
    # 断点续传配置
    START_INDEX = 1694 # 起始索引，从0开始。如果中断了，设置为下一个要处理的索引
    ...
    if MODE == 'test':
        ...
        # 生成用户画像
        results = generate_user_profiles(
            prompts_file=PROMPTS_FILE,
            system_prompt_file=SYSTEM_PROMPT_FILE,
