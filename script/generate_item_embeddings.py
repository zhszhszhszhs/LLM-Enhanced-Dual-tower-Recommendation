...
def generate_embeddings(profile_file, output_file, model='text-embedding-v2', 
    ...
    Returns:
        numpy.array: 所有嵌入向量 [n_items, embedding_dim]
    """
    print("=" * 60)
    print("生成物品画像嵌入向量")
    print("=" * 60)
    ...
    # 1. 加载物品画像
    print(f"\n[1/3] 加载物品画像: {profile_file}")
    profiles = load_profiles(profile_file)
    print(f"   物品总数: {len(profiles)}")
    ...
    for i in tqdm(range(start_index, len(profiles)), desc="生成进度"):
        profile = profiles[i]
        item_index = profile.get('item_index', i)
        ...
        if not summarization or summarization == 'None':
            ...
            if embedding is None:
                print(f"\n   错误: 物品{item_index}的嵌入生成失败，使用零向量")
                embedding = np.zeros(embedding_dim, dtype=np.float32)
                failed_count += 1
                failed_indices.append(item_index)
            else:
        ...
        if embedding.shape[0] != embedding_dim:
            print(f"\n   错误: 物品{item_index}的嵌入维度错误({embedding.shape[0]}!={embedding_dim})，使用零向量")
            embedding = np.zeros(embedding_dim, dtype=np.float32)
            failed_count += 1
            failed_indices.append(item_index)
...
def test_single_embedding(profile_file, item_index=None):
    """
    测试单个物品画像的嵌入生成
    ...
    if item_index is None:
        item_index = np.random.choice(len(profiles))
    
    profile = profiles[item_index]
    profile_data = profile.get('profile', {})
    summarization = profile_data.get('summarization', '')
    
    print(Colors.GREEN + "=" * 60 + Colors.END)
    print(Colors.GREEN + f"测试物品嵌入生成 (物品索引: {item_index})" + Colors.END)
    print(Colors.GREEN + "=" * 60 + Colors.END)
...
if __name__ == '__main__':
    # 配置文件路径
    # PROFILE_FILE = 'dataset/toy/processed/item_profiles.json'
    # OUTPUT_FILE = 'dataset/toy/processed/item_embeddings.npy'

    PROFILE_FILE = 'dataset/sports/processed/item_profiles.json'
    OUTPUT_FILE = 'dataset/sports/processed/item_embeddings.npy'    
    # 配置参数
    ...
    if MODE == 'test':
        ...
        print("")
        print("# 加载嵌入向量")
        print(f"embeddings = np.load('{OUTPUT_FILE}')")
        print("print(f'形状: {embeddings.shape}')  # (7695, 1536)")
        print("")
        print("# 获取第0个物品的嵌入")
        print("item_0_emb = embeddings[0]")
        print("print(item_0_emb.shape)  # (1536,)")
        print("")
        print("# 计算相似度")
        print("from sklearn.metrics.pairwise import cosine_similarity")
        print("sim = cosine_similarity([embeddings[0]], [embeddings[1]])")
        print("print(f'物品0和物品1的相似度: {sim[0][0]:.4f}')")
        print("```")
